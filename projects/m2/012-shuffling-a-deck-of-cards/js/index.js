...
function createDeck(){
  ...
  const suitOfTheCard = ["s", "h", "d", "c"];
  const listCards = [];

  for(let i = 0; i < suitOfTheCard.length; i++){
    for(let y = 0; y < cards.length; y++){
      listCards.push(cards[y] + suitOfTheCard[i]);
    }
  }
...
function shuffle(deck){
  ...
  while(shuffledCards.length < 52){
    ...
    if(!shuffledCards.includes(deck[randomNumber])){
      shuffledCards.push(deck[randomNumber]);
    }
  }
