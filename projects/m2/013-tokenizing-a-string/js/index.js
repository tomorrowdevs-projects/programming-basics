...
const stringToConvert = "1*(2 + (6 * (3-1)) / 844 + (74 ^ 3)) / 121-10+10";
const result = convertString(stringToConvert);
console.log(result);
...
function convertString(stringToConvert) {
  ...
  for (let i = 0; i < stringToConvert.length; i++) {
    const currentChar = stringToConvert[i];

    if (currentChar !== " ") {
      if (listChar.includes(currentChar)) {
        if (elementString !== "") {
          convertedString.push(elementString);
          elementString = "";
        }
        convertedString.push(currentChar);
      } else {
