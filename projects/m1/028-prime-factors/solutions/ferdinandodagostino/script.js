...
let factor = 2;
const primeFactors = [];
let userNum = prompt('Inserisci un numero maggiore di 2');

do {
  if (userNum < factor) {
    alert('Inserisci un numero maggiore di 2');
  } else if (userNum % factor == 0) {
    primeFactors.push(factor);
    userNum = Math.floor(userNum / factor);
  } else {
    factor++;
  }
} while (factor <= userNum);
alert(primeFactors.toString());
